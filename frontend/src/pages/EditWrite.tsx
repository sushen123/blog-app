...
export const Edit = () => {
    ...
    const [story, setStory] = useState(state?.content || "");
    
    
    const [publish, setPublish] = useState(state.pubBool)
    ...
    useEffect(() => {
     ...
     if(response.status === 200) {
        setAlert("Published Successfully!")
     }
     setPublish(true)
     ...
     if(response.status === 200) {
        setAlert("UnPublished Successfully!")
     }
     setPublish(false)
     setPubLoadiing(false)
       }
    ...
    <div className="flex border-b justify-between">
        ...
        <button onClick={() => {
            navigate("/blog")
        }} className="font-bold text-3xl sm:text-5xl " >TW</button>
        <div className="flex ml-5 h-full mt-2 items-center ">
           <button onClick={() => {
           handle()
           
           }} className={  ` ${saveLoading ? "hidden": "flex"} bg-blue-500 px-3 py-1 rounded-full hover:bg-opacity-50`}> Update </button>
...
Loading...
        ...
        <div className={`flex items-center mt-2` }>
            ...
            }} className={`bg-green-400 p-1 px-3 rounded-full hover:bg-opacity-50 ${publoading ? "hidden" : "flex"}`}>{publish ? "UnPublish": "Publish"}</button>
...
Loading...
        ...
        <button onClick={handleProfileClick} className="flex m-2 mx-5 items-center w-8 h-8 justify-center  rounded-full bg-cyan-400 hover:bg-opacity-50" ref={profileRef}>
         S
        </button>
